 ...
 * produces a {@code Map<Long, RowIdMapping>} that secondary formats use to align their
 * document order.
 *
 * <p>When Lucene is a <b>secondary</b> format, it receives a {@code Map<Long, RowIdMapping>}
...
public interface LuceneMergeStrategy {
     ...
     * @param rowIdMappings the row ID mappings from the primary format keyed by generation, or null if this is the primary
     * @return the configured OneMerge for execution
     */
    MergePolicy.OneMerge createOneMerge(List<SegmentCommitInfo> segments, Map<Long, RowIdMapping> rowIdMappings);

    /**
     * Builds or resolves the {@code Map<Long, RowIdMapping>} after the merge completes.
     *
     ...
     * @return the row ID mappings for the merge result, or null if not applicable
